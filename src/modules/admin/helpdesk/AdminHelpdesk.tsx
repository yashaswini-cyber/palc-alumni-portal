...
type ConversationMessage = { id: string; sender: "Employee" | "Support"; message: string; time: string; };
type HelpdeskTicket = { id: string; subject: string; category: string; priority: string; status: string; createdOn: string; updatedOn: string; description?: string; attachment?: File | null; requiredBy?: Date | null; conversation?: ConversationMessage[]; };

const tickets: HelpdeskTicket[] = [
  { id: "TKT001", subject: "Unable to access VPN", category: "IT", priority: "High", status: "Open", createdOn: "08 Jul 2026", updatedOn: "08 Jul 2026", description: "Unable to establish VPN connection after changing my password.", requiredBy: null, attachment: null, conversation: [{ id: "1", sender: "Employee", message: "Unable to establish VPN connection after changing my password.", time: "08 Jul 2026 • 09:15 AM" }, { id: "2", sender: "Support", message: "We've received your request. Our IT team is currently investigating the issue.", time: "08 Jul 2026 • 09:40 AM" }] },
  { id: "TKT002", subject: "Payslip not available", category: "Payroll", priority: "Medium", status: "Resolved", createdOn: "05 Jul 2026", updatedOn: "07 Jul 2026", description: "June payslip is missing from the employee portal.", requiredBy: null, attachment: null, conversation: [{ id: "1", sender: "Employee", message: "My June payslip is not visible in the portal.", time: "05 Jul 2026 • 11:20 AM" }, { id: "2", sender: "Support", message: "The payroll system has been refreshed. Please check again.", time: "05 Jul 2026 • 01:05 PM" }, { id: "3", sender: "Employee", message: "I can see it now. Thank you.", time: "05 Jul 2026 • 01:18 PM" }] },
  { id: "TKT003", subject: "Email password reset", category: "IT", priority: "Low", status: "In Progress", createdOn: "04 Jul 2026", updatedOn: "06 Jul 2026", description: "Need to reset my corporate email password.", requiredBy: null, attachment: null, conversation: [{ id: "1", sender: "Employee", message: "I forgot my Outlook password.", time: "04 Jul 2026 • 03:40 PM" }, { id: "2", sender: "Support", message: "Your password reset request has been assigned to the IT team.", time: "04 Jul 2026 • 04:05 PM" }] }
];

const helpdeskStats = { totalTickets: tickets.length, openTickets: tickets.filter((t) => t.status === "Open").length, inProgressTickets: tickets.filter((t) => t.status === "In Progress").length, resolvedTickets: tickets.filter((t) => t.status === "Resolved").length };
const statsCards = [
...
const relatedArticles: Record<string, string[]> = {
  "IT Support": ["VPN Connection Issues", "Reset Corporate Password", "Configure Outlook"],
  HR: ["Leave Policy", "Employee Benefits", "HR Portal Guide"],
  Payroll: ["Payslip Download", "Tax Declaration", "Salary Credit FAQ"],
  Benefits: ["Insurance Enrollment", "Medical Claims", "Wellness Benefits"],
  Documents: ["Download Experience Certificate", "Request Form 16", "Document Access Guide"],
  "Employment Verification": ["Employment Verification Process", "Generate Verification Letter", "Verification FAQs"],
  Accounts: ["Expense Reimbursement", "Travel Claims", "Finance Help"],
  Other: ["General Support", "Helpdesk Guidelines", "Contact Support"]
};
...
export default function HelpdeskPage() {
  ...
  const handleTicketChange = (field: keyof typeof ticketForm, value: string | File | null) => setTicketForm((prev) => ({ ...prev, [field]: value }));
  const resetTicketForm = () => { setTicketForm({ category: "IT Support", priority: "Medium", subject: "", description: "", attachment: null }); setRequiredBy(null); };

  useEffect(() => { saveHelpdeskTickets(ticketList); }, [ticketList]);
  const ticketsSectionRef = useRef<HTMLDivElement>(null);

  const openCreateTicket = () => document.getElementById("create-ticket")?.scrollIntoView({ behavior: "smooth", block: "start" });
  const cancelCreateTicket = () => resetTicketForm();

  const handleSubmitTicket = () => {
    if (!ticketForm.subject.trim() || !ticketForm.description.trim() || !requiredBy) { alert("Please complete all mandatory fields."); return; }
    const ticketId = `HD-${Date.now()}`;
    const newTicket: HelpdeskTicket = { id: ticketId, subject: ticketForm.subject, category: ticketForm.category, priority: ticketForm.priority, description: ticketForm.description, attachment: ticketForm.attachment, requiredBy, status: "Open", createdOn: new Date().toLocaleDateString(), updatedOn: "Just now", conversation: [{ id: crypto.randomUUID(), sender: "Employee", message: ticketForm.description, time: new Date().toLocaleString() }] };
  ...
  const openTicketDetails = (ticket: HelpdeskTicket) => { setSelectedTicket(ticket); setShowTicketDetails(true); };
  const closeTicketDetails = () => { setReplyMessage(""); setShowTicketDetails(false); setSelectedTicket(null); };

  const sendReply = () => {
    if (!selectedTicket || !replyMessage.trim()) return;
    const employeeReply = { id: crypto.randomUUID(), sender: "Employee" as const, message: replyMessage.trim(), time: new Date().toLocaleString() };
    ...
    const updateConversation = (messages: ConversationMessage[]) => {
      const updated = ticketList.map((t) => (t.id === selectedTicket.id ? { ...t, conversation: messages, updatedOn: "Just now" } : t));
      setTicketList(updated);
      setSelectedTicket(updated.find((t) => t.id === selectedTicket.id) || null);
    };

    const employeeConversation = [...(selectedTicket.conversation || []), employeeReply];
    updateConversation(employeeConversation);
    setReplyMessage("");
    setTimeout(() => {
      const supportReply = { id: crypto.randomUUID(), sender: "Support" as const, message: supportReplies[selectedTicket.category] || supportReplies.Other, time: new Date().toLocaleString() };
      updateConversation([...employeeConversation, supportReply]);
    }, 2500);
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
      <HeroBanner badge="Employee Helpdesk" title="Support When You Need It" subtitle="Submit support requests, track ticket progress, communicate with support teams, and quickly find answers through the knowledge base—all from one centralized workspace." actions={[{ title: "Create Ticket", onClick: openCreateTicket }, { title: "View My Tickets", onClick: scrollToTickets }]} pills={[{ title: "Response SLA", value: "< 24 Hours", color: "#22C55E" }, { title: "Support", value: "IT • HR • Payroll", color: "#3B82F6" }]} summaryCard={<><p style={{ margin: 0, fontSize: "13px", fontWeight: 700, color: "#64748B", textTransform: "uppercase", letterSpacing: ".06em" }}>Support Overview</p><h3 style={{ margin: "10px 0 6px", fontSize: "36px", fontWeight: 800 }}>96%</h3><p style={{ margin: 0, color: "#64748B", lineHeight: 1.6 }}>First-response SLA achieved across all employee support requests this month.</p><div style={{ marginTop: "22px", paddingTop: "18px", borderTop: "1px solid #E2E8F0", display: "flex", justifyContent: "space-between" }}><div><div style={{ fontSize: "13px", color: "#64748B" }}>Avg. Resolution</div><div style={{ fontSize: "18px", fontWeight: 700 }}>18 hrs</div></div><div><div style={{ fontSize: "13px", color: "#64748B" }}>Satisfaction</div><div style={{ fontSize: "18px", fontWeight: 700 }}>4.9 / 5</div></div></div></>} />

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "20px" }}>
        {statsCards.map((card) => (<StatsCard key={card.title} title={card.title} value={card.value} subtitle={card.subtitle} accentColor={card.accentColor} />))}
      </div>
      ...
      <div id="create-ticket">
        <SectionCard title="Create a New Support Ticket" subtitle="Provide the information below to help our support team understand and resolve your issue efficiently.">
          ...
          <div style={{ marginTop: 28 }}>
            ...
            <textarea rows={5} placeholder="Describe the issue in detail." value={ticketForm.description} onChange={(e) => handleTicketChange("description", e.target.value)} style={{ ...inputStyle, resize: "vertical", fontFamily: "inherit", fontSize: "14px" }} />
          </div>
          ...
          <div style={{ marginTop: 28 }}>
            ...
            <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "10px", minHeight: "160px", border: `2px dashed ${COLORS.border}`, borderRadius: "14px", background: "#F8FAFC", cursor: "pointer", transition: ".2s", textAlign: "center", padding: "28px" }}>
              ...
              {ticketForm.attachment && <div style={{ marginTop: "8px", padding: "8px 14px", borderRadius: "999px", background: "#DBEAFE", color: "#1D4ED8", fontWeight: 600, fontSize: "13px" }}>{ticketForm.attachment.name}</div>}
            ...
            <div style={{ marginTop: "12px", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "10px", color: COLORS.textSecondary, fontSize: "13px" }}>
              <span><strong>Supported:</strong> PDF, PNG, JPG, DOC, DOCX</span>
          ...
          <div style={{ marginTop: "30px", border: `1px solid ${COLORS.border}`, borderRadius: "14px", padding: "22px", background: "#FFFFFF" }}>
            <h4 style={{ margin: "0 0 14px", color: COLORS.text, fontSize: "17px" }}>Related Help Articles</h4>
            <p style={{ margin: "0 0 18px", color: COLORS.textSecondary, lineHeight: 1.6 }}>These articles may help resolve your issue before submitting a support request.</p>
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {relatedArticles[ticketForm.category]?.map((article) => (
                <div key={article} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 14px", borderRadius: "10px", background: "#F8FAFC", border: `1px solid ${COLORS.border}`, cursor: "pointer", transition: ".2s" }}>
        ...
        <SectionCard title="My Support Tickets">
          <div style={{ marginBottom: "22px" }}>
            <div style={{ display: "flex", gap: "18px", flexWrap: "wrap", alignItems: "center" }}>
              <div style={{ flex: 1, minWidth: "320px" }}><SearchBar value={searchQuery} placeholder="Search Ticket, ID, Name, or Category" onChange={(e) => setSearchQuery(e.target.value)} /></div>
                    ...
                    <tr key={ticket.id} style={{ borderBottom: `1px solid ${COLORS.border}`, transition: "background .2s ease" }} onMouseEnter={(e) => { e.currentTarget.style.background = "#F8FAFC"; }} onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}>
                      ...
                      <td style={{ padding: "12px" }}>
                        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                          ...
                          {ticket.status === "Resolved" && <PrimaryButton onClick={() => reopenTicket(ticket.id)}>Reopen</PrimaryButton>}
                        </div>
                      </td>
                    </tr>
                  ))
        ...
        {selectedTicket && (
          <div>
                  ...
                  <div key={msg.id} style={{ display: "flex", justifyContent: msg.sender === "Employee" ? "flex-end" : "flex-start" }}>
                    <div style={{ maxWidth: "78%", padding: "14px 16px", borderRadius: "14px", background: msg.sender === "Employee" ? "#E0F2FE" : "#F8FAFC", border: `1px solid ${COLORS.border}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", gap: "16px", marginBottom: "6px" }}>
                        <span style={{ fontWeight: 700, fontSize: "13px" }}>{msg.sender}</span>
                        <span style={{ fontSize: "11px", color: COLORS.textSecondary }}>{msg.time}</span>
                      </div>
                      <div style={{ fontSize: "14px", lineHeight: 1.6, color: COLORS.text }}>{msg.message}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <textarea rows={3} placeholder="Type your reply here..." value={replyMessage} onChange={(e) => setReplyMessage(e.target.value)} style={{ ...inputStyle, resize: "vertical", fontFamily: "inherit" }} />
              <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px" }}>
                <button onClick={closeTicketDetails} style={{ padding: "10px 20px", borderRadius: "8px", border: `1px solid ${COLORS.border}`, background: "#FFF", cursor: "pointer", fontWeight: 600 }}>Close</button>
