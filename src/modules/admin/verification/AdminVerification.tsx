...
const handleReject = (requestId: string) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "32px", padding: "20px" }}>
                  ...
                  <tr key={req.id} style={{ borderBottom: `1px solid ${COLORS.border}`, transition: "0.2s ease" }} onMouseEnter={(e) => (e.currentTarget.style.background = "#F8FBFF")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                    ...
                    <td style={{ padding: "12px" }}>
                      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                        ...
                        {req.status === "Approved" ? (
                          <PrimaryButton>View Decision</PrimaryButton>
                        ) : (
                          <button disabled style={{ opacity: 0.5, cursor: "not-allowed", padding: "8px 14px", borderRadius: "8px", border: `1px solid ${COLORS.border}`, background: "#F8FAFC", color: COLORS.textSecondary }}>Pending Review</button>
                        )}
                      </div>
                    </td>
                  </tr>
     ...
     <SectionCard
      title="Employment Verification Resources"
      subtitle="Access the official employment documents and templates used during employment verification and HR review.">
          ...
          {[
  {
    ...
    status: "Available",
    issued: "Updated July 2026",
    documentId: "TEMP-001",
    description: "Standard employment certificate template used during employment verification."
  },
  {
    ...
    issued: "Updated July 2026",
    documentId: "TEMP-002",
    description: "Official experience letter template used for employee verification."
  },
  {
    ...
    issued: "Updated July 2026",
    documentId: "TEMP-003",
    description: "Standard relieving letter template used for verification requests."
  },
].map((document) => (
            <SectionCard key={document.documentId} title={document.title}>
              <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
                ...
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginTop: "6px" }}>
                  ...
                  <button style={secondaryButtonStyle}>View Template</button>
                </div>
              </div>
            </SectionCard>
          ))}
    ...
    <div id="verification-guidelines">
      <SectionCard
        title="Verification Guidelines"
        subtitle="Follow the standard employment verification process to ensure every request is reviewed accurately and consistently.">
        <div style={{ display: "grid", gridTemplateColumns: "1.3fr 0.9fr", gap: "32px", alignItems: "start" }}>
          <div>
            <h3 style={{ margin: "0 0 14px", color: COLORS.text }}>HR Verification Checklist</h3>
            <p style={{ color: COLORS.textSecondary, lineHeight: 1.7, marginBottom: "24px" }}>Review each request carefully before approving or rejecting it. Ensure employment records are accurate and complete.</p>
            {[
              {
                title: "Verify Employee Identity",
                text: "Confirm employee information before processing the request."
              },
              {
                title: "Validate Employment Records",
                text: "Review employment dates, designation and department."
              },
              {
                title: "Review Supporting Documents",
                text: "Ensure all required employment documents are available."
              },
              {
                title: "Approve or Reject",
                text: "Complete the verification with an appropriate decision."
              }
            ].map((item) => (
          ...
          <div style={{ background: "#FFFFFF", border: `1px solid ${COLORS.border}`, borderRadius: "18px", padding: "24px" }}>
            <h3 style={{ marginTop: 0, color: COLORS.text }}>Verification Summary</h3>
            <div style={{ display: "grid", gap: "18px", marginTop: "22px" }}>
              {[
                ["Pending Requests", pendingRequests.length.toString()],
                ["Approved Requests", approvedRequests.length.toString()],
                ["Rejected Requests", rejectedRequests.length.toString()],
                ["Completed", completedRequests.length.toString()]
              ].map(([label, value]) => (
                <div key={label}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginTop: "28px" }}>
              <PrimaryButton>Export Verification Report</PrimaryButton>
              <button style={secondaryButtonStyle}>Refresh Requests</button>
            </div>
            <div style={{ marginTop: "24px", padding: "14px", borderRadius: "12px", background: "#EFF6FF", border: `1px solid ${COLORS.border}` }}>
              <strong style={{ color: COLORS.primary }}>HR Notice</strong>
              <p style={{ margin: "8px 0 0", color: COLORS.textSecondary, lineHeight: 1.6 }}>Verification decisions are securely recorded and will automatically sync with the Alumni Portal after backend integration.</p>
            </div>
          </div>
        </div>
      </SectionCard>
            ...
            {[
              {
                ...
                description: "A verification request is submitted by an alumnus.",
                color: "#2563EB"
              },
              {
                title: "Review Employment Records",
                description: "HR validates employment information and supporting records.",
                color: "#0EA5E9"
              },
              {
                title: "Validate Documents",
                description: "Supporting documents are reviewed for authenticity.",
                color: "#D97706"
              },
              {
                title: "Approve or Reject",
                description: "HR records the final verification decision.",
                color: "#9333EA"
              },
              {
                title: "Completed",
                description: "The request is completed and the status is updated.",
                color: "#16A34A"
              }
            ].map((step) => (
